...
public class PageFind  extends Page {
   ...
   */
	...
	private void Addraw(Recipe recipe,int n){
		dtm.addRow(new Object[]{recipe.getName(), recipe.getAuthor(), recipe.getCook_time(),recipe.getDifficulty(),"   "});
		listnubers.add(new Integer(n));
	}
	//Add new raw
	private void Addraw(Recipe recipe,int n,int i){
		dtm.addRow(new Object[]{recipe.getName(), recipe.getAuthor(), recipe.getCook_time(),recipe.getDifficulty(),recipe.getIngredients().get(i).getName()});
		listnubers.add(new Integer(n));
	}
        //find with keywords
